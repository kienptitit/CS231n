...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(x @ Wx + prev_h @ Wh + b[None,:])
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(t):
        ...
        h_tmp.append(next_h[:,None,:])
        cache.append(c)

    h = np.concatenate(h_tmp,axis = 1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    dx_step, dh_step , dWx_step, dWh_step, db_step = rnn_step_backward(dh[:,-1,:],cache[-1])

    dWx = dWx_step.copy()
    dWh = dWh_step.copy()
    db = db_step.copy()

    dx_tmp.append(dx_step[:,None,:])

    for i in range(t - 2, -1, -1):

        dx_step, dh_step , dWx_step, dWh_step, db_step = rnn_step_backward(dh_step + dh[:,i,:],cache[i])
    ...
    dh0 = dh_step
    dx = np.concatenate(dx_tmp[::-1],axis = 1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
